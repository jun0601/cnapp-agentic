...
## 🗺️ 1. 전체 그림 — 5단계 파이프라인 (2-pass, project-draft §4.4)
...
> **✅ 2026-07-02 — 실 스왑 배포판 작성됨:** [`engine/handler.py`](handler.py)(Lambda 진입점 — `REAL_TOOLS=1`이면 `RealToolExecutor`+`BedrockEvidenceAgent`, `run_real.py`와 동일 구성). RDS에서 finding/attack_path 로드 → `cases`·`finding_explanations` upsert. 스키마 = [`infra/shared/db/schema.sql`](../infra/shared/db/schema.sql), 배포 = [`infra/backend`](../infra/backend/). Phase1에서 tool-use 자체는 실검증 완료(`run_real`).

---

## 🔗 7. attackpath와의 연결 (엔진의 입력은 어디서 오나)

이 엔진의 ① Triage 게이트가 승급 조건으로 쓰는 `attack_path_id`는 **[attackpath](../attackpath/README.md) 영역이 먼저 찍어준다.** 두 영역은 이렇게 이어진다:
...
즉 **attackpath가 "여기 위험한 조합이 있다"고 먼저 찾아주면 → 엔진이 "그게 진짜인지" 파고드는** 협업 구조다. 지금은 두 데모(`engine.run_demo` · `attackpath.run_demo`)가 따로 돌지만, 실배포에선 `cnapp.attackpath.correlation.completed` 이벤트로 이 순서가 이어진다(project-draft §4.4).
