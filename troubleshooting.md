...
- `2026-07-01 / 준형 / [apps-target] member를 'shop 데모 포털'로 확장 — /(포털: 설명+3서비스 카드+골든 경로)·/product·/order(소개+결함표)·/members(회원 관리 UI). JSON API는 /api/members로 이동(HTML /members와 경로 충돌 회피). 쇼핑백 파비콘(/favicon.svg) 추가. 관제 앱 파비콘은 밝은 파랑 방패로 교체(밝은 방패 이모지 느낌, 크로스클라우드 노드 특이성 유지). 4페이지+파비콘+API 전부 200 확인. product/order는 로컬 미구동이라 소개 페이지로(실구동 EKS)`
- `2026-07-01 / 준형 / [engine] 엔진 Evidence·Triage 목업 능동조사 구현(python -m engine.run_demo) — 트리아지 게이트(20건→12 승급, 비용통제) → Evidence가 계약④ allowlist 강제하며 read-only 툴 4회 자가 호출(챗봇 탈출) → confirmed 판정 → case(계약⑦) 스키마·골든 정합 검증. MockToolExecutor·규칙 플래너(control→tool)라 RealToolExecutor(boto3)·Bedrock LLM로 갈아끼우면 실AWS 전환(로직 무변). 독립 리뷰 반영: _next_ts 타임스탬프 wraparound(초 %60 → timedelta 캐리), validate_shape에 evidence항목·verdict enum 검증 추가, triage reason 실드라이버만·run_demo reason 도출·plan resource_id/빈 attack-path 가드. 3.8 호환·allowlist 차단(s3:DeleteBucket 거부) 확인`
