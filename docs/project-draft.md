...
| **스캐너-CSPM** | CSPM(Config · Prowler · Security Hub · **Macie(AWS S3 전용)**) | 워크로드(Inspector · Trivy · kube-bench) | 확정 |
...
| 점검 깊이 | 풀 CNAPP 6기둥 | **Entra CIEM**(과도권한 앱 등록·위험한 consent·권한상승) — Defender for Cloud는 시도 후 범위 제외(D11) |
| 비중 | 80% | 20% |
...
- 스캐너(Config·Prowler·Inspector·Trivy·kube-bench·Macie) → 계정을 **read-only 스캔** → findings.
...
| D11 | **Azure = Entra CIEM 중심**(Defender for Cloud는 범위 제외, 2026-07-07) | Prowler entra_id_* 체크로 과도권한 앱·위험한 consent·권한상승 탐지(핵심). Security Hub는 AWS 전용. **Defender for Cloud는 시도했으나 드롭** — 이 프로젝트의 Azure 자산이 Entra ID 신원 객체뿐(VM·스토리지 등 실 리소스 0개)이라, Defender for Cloud의 CSPM 엔진(리소스 인벤토리 기반)이 평가할 대상 자체가 없어 권한을 다 열어도 secure score가 영구히 "해당없음"으로 남음(실측 확인, 상세는 troubleshooting.md 2026-07-07). 콘솔 Scores 화면의 Azure 점수는 대신 실 open findings 심각도 가중 페널티로 산출(`console-backend/data.ts getScores()`). |
...
[입력: CNAPP 신호]
  ...
  Azure:      Prowler(Azure 모드 — OCSF 출력 → S3 → EventBridge → 기존 파이프라인 합류)
              ...
              (Defender for Cloud는 시도 후 범위 제외 — 실 Azure 리소스 0개라 CSPM 평가 대상 없음, D11)
...
| ~~제외~~ | ~~Defender for Cloud~~ | ~~Azure 리소스 CSPM~~ | **2026-07-07 범위 제외** — CSPM 엔진이 리소스 인벤토리 기반인데 이 프로젝트 Azure 자산은 Entra ID 신원 객체뿐(실 리소스 0개)이라 평가 대상이 없어 secure score가 영구히 미집계로 확인(권한 이슈 아님, 실측). Azure Scores는 대신 실 open findings 기반 산출(§4.4.1). 상세는 troubleshooting.md |
| 선택 | Azure Policy / Log Analytics Workspace | 리소스 규정 준수 평가·로그 수집 | Defender for Cloud 연계 목적이었던 항목이라 위 결정에 따라 미착수 유지 |
