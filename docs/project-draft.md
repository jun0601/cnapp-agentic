...
| 영역 | 트랙1 | 트랙2 |
|---|---|---|
| **앱** | 타깃 앱(결함 심기) | 관제 앱(대시보드·시각화) |
| **토대** | CI/CD · Shift-Left · **공유인프라 주도** | 모니터링 · 관제 · 추적(Grafana · CloudTrail) |

### 4.2 핵심 영역 분담 (각 영역 반반 — ⚠️ 트랙2 협의 후 최종 확정)

| 영역 | 트랙1 | 트랙2 |
|---|---|---|
| **스캐너** | CSPM: Config · Prowler · Security Hub · **Macie(AWS S3 전용)** | 워크로드: Inspector · Trivy · kube-bench · **Defender(Azure 리소스)** |
| **수집·정규화** | 수집부: EventBridge→SQS | 정규화부: Lambda→OCSF |
| **에이전틱 엔진(Bedrock)** | **Evidence(tool use)** · Triage | Hypothesis · **Reasoning** · Orchestrator |
| **RAG** | 코퍼스 · 임베딩 · pgvector 적재 | 검색 · LLM 답변 생성 |
| **attack-path** | 그래프 데이터 모델 | 상관 로직 · 내러티브 |

> CIEM은 양쪽에 걸침: AWS IAM(트랙2 워크로드 권한) + Azure Entra 신원(과도권한 앱·위험한 consent)은 attack-path 상관과 함께 본다.

### 4.3 의존성 순서 (병목 주의 — 데이터 흐름 방향이라 앞 단계가 있어야 뒤가 된다)

```
0. 공유인프라        (트랙1·최우선 — 없으면 둘 다 멈춤)
1. 앱배포(트랙1)      ∥  모니터링 골격(트랙2)
2. 스캐너 트랙1=CSPM  ∥  트랙2=워크로드
3. 수집(트랙1)        →  정규화(트랙2)            ← 순차(앞이 있어야 뒤)
4. RAG토대 트랙1=적재 ∥  트랙2=검색연동
5. 엔진 트랙1=Evidence ∥ 트랙2=Reasoning
6. 출력 트랙1=CI/CD·SSO ∥ 트랙2=관제앱·시각화
7. 골든 시나리오 데모  (둘이 합류)
```

`∥` = 병렬 가능, `→` = 순차(앞 단계 산출물 필요). **0번 공유인프라는 트랙1이 최우선으로 깐다** — 안 되면 양쪽 모두 블록.

### 4.4 미리 합의할 인터페이스 2개 (이것만 정하면 나머지는 병렬 가능)

1. **OCSF 스키마 (3단계)** — findings 정규화 형식. 트랙1·트랙2·관제앱이 **공통으로 쓰는 계약**. 수집↔정규화↔대시보드가 전부 이걸 읽고 쓴다.
2. **엔진 입출력 형식 (5단계)** — Evidence가 모은 증거를 Reasoning이 받는 형식. **이 지점이 프로젝트의 심장** — 에이전틱 능동조사(트랙1 Evidence) → attack-path 추론(트랙2 Reasoning)이 만나는 곳.
...
- **AWS:** 커머스 마이크로서비스 3~4개(상품·주문·회원·결제) + EKS. **회원 PII는 AWS S3 버킷에만 보관.**
...
*변경 요약(v5.2): **작업 분담을 균형안으로 갱신**(4번) — 각 영역(스캐너·수집·엔진·RAG·attack-path)을 반반 갈라 양쪽이 핵심을 다 만지게. 의존성 순서(4.3)·합의 인터페이스 2개(OCSF 스키마·엔진 입출력, 4.4)·시간 컷 우선순위(엔진 능동조사 사수, 4.5) 추가. 로드맵(21번)을 이 분담·우선순위와 정합되게 갱신. 핵심 영역 분담은 트랙2 협의 후 최종 확정.*
