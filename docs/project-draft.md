...
**검증에서 새로 드러난 미확정 (contracts/ 졸업 단계에서 진우와 확정):**
- [ ] **`resource_id` 정규화 규칙** — `{cloud}:{type}:{native_id}`의 `native_id` 표준형을 resource_type별로 고정(예: S3=버킷명, IAM=role명). 스캐너마다 같은 리소스를 다르게 부르면(ARN vs 이름) `dedup_key`가 안 맞아 중복/상태매칭 실패 → 정규화부가 캐논화. 계약①에 박는다.
- [ ] **INTERNAL `control_id` 카탈로그 + 스캐너 매핑표** — `securityhub:S3.8`·`config:...`·`prowler:...` → 단일 `INTERNAL-*` id 매핑표가 아직 없음. 정규화·RAG 조인(`metadata.control_id`)·mock-findings가 모두 이걸 전제 → `contracts/`에 카탈로그 생성.
- [ ] **remediated 판정 스코프(스캐너별)** — 계약⑤ "배치에 없으면 remediated"를 이종 스캐너(Config 상시·Prowler 일일·Trivy는 CI·Inspector)에 그대로 쓰면 "Trivy 배치에 없으니 Config finding을 remediated" 식 오판. 판정을 *(그 스캐너가 커버하는 control)* 스코프로 한정해야 함 → 정규화·상태동기화(console 6.1)와 함께 확정.
