...
아래 표는 두 트랙이 각각 맡는 새 역량·보안 영역·핵심 기술을 정리한 것이다.
...
- **크로스클라우드 attack-path** (위 1번의 심화) — AWS 침해가 Azure 신원·데이터로 번지는 경로를 추적.
  - *대비:* 대부분은 **단일 클라우드 내부**(특히 AWS) 상관에 머문다. 우리는 **AWS→Azure 경계를 넘는 탈취 경로**를 하나의 그래프로 잇는다.
...
## 6. 🏗️ 멀티클라우드 구도 — AWS(워크로드) / Azure(신원·데이터)
