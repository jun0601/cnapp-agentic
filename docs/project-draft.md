...
>
> 상태: 설계 확정 / **구현 진행 중** — 공통 계약(14 control·목업·CI)·infra/shared·infra/target 스캐폴드, 관제 앱 8화면 목업 동작, 타깃 앱 member 실행 + shop 포털, **엔진 5단계 능동조사·attack-path 상관·정규화부(전부 목업 동작)**. 다음 = 실데이터 전환(엔진 실 tool-use 우선 → 스캐너·수집)
