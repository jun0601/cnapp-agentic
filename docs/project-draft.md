...
| **스캐너** | CSPM(Config · Prowler · Security Hub · **Macie(AWS S3 전용)**) | 워크로드(Inspector · Trivy · kube-bench · **Defender(Azure 리소스)**) | 협의중 |
...
> CIEM은 양쪽에 걸침: AWS IAM(진우 워크로드 권한) + Azure Entra 신원(과도권한 앱·위험한 consent)은 attack-path 상관과 함께 본다.
...
1. **OCSF 스키마 (3단계)** — findings 정규화 형식. 준형·진우·관제앱이 **공통으로 쓰는 계약**. 수집↔정규화↔대시보드가 전부 이걸 읽고 쓴다.
2. **엔진 입출력 형식 (5단계)** — Evidence가 모은 증거를 Reasoning이 받는 형식. **이 지점이 프로젝트의 심장** — 에이전틱 능동조사(준형 Evidence) → attack-path 추론(진우 Reasoning)이 만나는 곳.
