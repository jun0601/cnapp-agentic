...
## 6. AWS 계정 플랜 + 보안 서비스 6종 — ✅ 활성 (2026-07-24, 담당: 준형)

### 6.1 계정 플랜: Free → Paid 전환 ⚠️ 되돌릴 수 없음

2026-07-06부터 Security Hub·Macie·Inspector·GuardDuty가 `SubscriptionRequiredException`으로 막혀 "계정 하자"로 판단하고 범위 제외했었으나, **2025-07-15 신설된 AWS 무료 플랜의 서비스 접근 제한**이 진짜 원인이었다(공식 문서: *Free account plans don't include access to AWS services and features that could possibly deplete your credits*).

**크레딧은 이월된다** — 플랜 비교표에 두 플랜 모두 `$100 가입 + 최대 $100 추가`가 동일하게 있고, *"Any remaining credits from your free account plan are automatically applied to future AWS bills"*. 유효기간도 플랜 무관 **가입일(2026-06-29)로부터 12개월**. 전환 후 사용액이 계속 전액 크레딧 상쇄되는 것으로 실측 확인.

| | Free plan (이전) | **Paid plan (현재)** |
|---|---|---|
| 서비스 접근 | 크레딧 소진 가능 서비스 차단 | 전체 + 단기 무료 체험 |
| 크레딧 소진 시 | 🔴 **계정 자동 폐쇄**(데이터 90일 후 삭제) | 종량제 과금 시작 |

> 🚫 **AWS Organizations 절대 금지** — 가입하면 강제 업그레이드되며 *"you will no longer be able to use or earn credits"*. **크레딧을 잃는 유일한 경로**다(원래도 무료 크레딧 소멸 트리거로 금지 항목).

### 6.2 활성화한 보안 서비스 (전부 CLI, Terraform 관리 아님)

| 서비스 | 식별자 | 무료 체험 만료 |
|---|---|---|
| Security Hub | FSBP 표준 구독 | ~2026-08-23 |
| GuardDuty | detector `6acfc902580c3e1bd5cc260e88f5d47d` | ~2026-08-23 |
| Macie | + 커스텀 식별자 `cnapp-korea-rrn` (`282acdde-d6e2-4ea8-bab1-aeffa02b7581`) | ~2026-08-23 |
| Detective | graph `…:graph:2f40777d94814deaaea0b68118e39afa` | ~2026-08-23 |
| Inspector | EC2 + ECR | **~2026-08-08** (가장 빠름) |
| **AWS Config** | 레코더 `default` | ❌ **체험 없음 — 첫날부터 과금** |

**Config 비용 절제**: 기록 대상을 `S3::Bucket`·`EC2::SecurityGroup`·`IAM::Role`·`IAM::Policy`·`ECR::Repository` **5종으로 한정**. ⚠️ `EC2::Instance`를 넣으면 Karpenter 노드 churn이 config item을 폭증시킨다. 부속 리소스도 전부 수동: S3 버킷 `cnapp-agentic-config-066107819776`(PAB+SSE+30일 lifecycle) · 서비스 연결 역할 `AWSServiceRoleForConfig`.

**Macie 커스텀 식별자를 만든 이유**: Macie 관리형 데이터 식별자에 **한국 주민등록번호가 없다**. 게다가 seeder의 합성 RRN은 체크섬이 200개 중 18개만 유효하고 이메일도 전부 `example.com`(RFC 2606 예약 테스트 도메인 — Macie 탐지 제외)이라 관리형으로는 0건이었다. 정규식(`[0-9]{6}-[1-4][0-9]{6}`) 기반 커스텀 식별자로 **RRN 200건 탐지 성공**.

**안전망**: Budget `cnapp-security-services` — 이 6개 서비스 합계가 **$1 초과 시 `bljh5220@gmail.com`로 메일**. 체험 종료를 즉시 감지한다.

### 6.3 🔴 destroy 시 반드시 함께 끌 것

**`deploy.ps1 -Action destroy`는 이 서비스들을 끄지 않는다** — 계정 레벨이라 Terraform state 밖이다. 스택을 다 내려도 Config는 계속 과금된다.

```bash
R=ap-northeast-2
aws configservice stop-configuration-recorder --configuration-recorder-name default --region $R   # 체험 없는 유일한 서비스 — 최우선
aws securityhub disable-security-hub --region $R
aws macie2 disable-macie --region $R
aws inspector2 disable --resource-types EC2 ECR --region $R
aws guardduty delete-detector --detector-id 6acfc902580c3e1bd5cc260e88f5d47d --region $R
aws detective delete-graph --graph-arn arn:aws:detective:ap-northeast-2:066107819776:graph:2f40777d94814deaaea0b68118e39afa --region $R
```

### 6.4 파이프라인 유입 스위치 (EventBridge)

Security Hub findings는 `cnapp-agentic-securityhub-imported` 규칙을 통해 자동으로 파이프라인에 유입된다(GuardDuty·Inspector·Macie도 Security Hub를 경유해 같은 경로). **비용이 튀면 이 규칙 하나로 유입을 차단**할 수 있다:

```bash
aws events disable-rule --name cnapp-agentic-securityhub-imported --region ap-northeast-2   # 끄기
aws events enable-rule  --name cnapp-agentic-securityhub-imported --region ap-northeast-2   # 켜기
```

> ⚠️ **2026-07-24 현재 DISABLED** — 활성화 직후 Bedrock 호출이 폭증해 파이프라인 수정(`normalize` 멱등성 + SQS 배치 창) 후 재개 대기 중. 수정은 배포·검증 완료됐으므로 켜도 되지만, 켠 뒤 30분간 `AWS/Bedrock Invocations` 추이를 확인할 것.
