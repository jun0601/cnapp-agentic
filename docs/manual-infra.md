...
4. **✅ Entra 엔티티 ID 확정(§3.6.5) — 2026-07-06 수정 완료:** Entra `cnapp-agentic-console-sso` 앱 SAML 식별자를 옛 값에서 **`urn:amazon:cognito:sp:ap-northeast-2_axVuh328G`**(현 라이브 Cognito Pool)로 교체. (회신 URL은 도메인 프리픽스 기반이라 불변.) ⚠️ **console을 destroy→재apply하면 Pool ID가 새로 발급**되므로 그때마다 이 식별자 재갱신 필요 — SSO를 자주 켤 거면 console 레이어는 유지 권장.
...
> **상태(2026-07-06):** 도메인·ACM·CloudFront·SPA OIDC·Cognito SAML·Entra 앱 할당·그룹 전부 라이브. **SSO 로그인 end-to-end 검증 완료**(`jh_lee@` → APPROVER, 실데이터 20건). 유일한 유지보수 포인트 = console를 destroy→재apply하면 Cognito Pool ID가 바뀌므로 Entra 앱 SAML 식별자 재갱신 필요(§5.4 #4).
