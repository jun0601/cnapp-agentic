...
## 5. 도메인 + SSO 활성 (관제 앱) — 🔄 진행 중 (담당: 준형 배선 / 도메인 구매·NS: 진우)
...
4. **Entra 엔티티 ID 확정(§3.6.5):** Entra `cnapp-agentic-console-sso` 앱의 SAML 식별자를 실 Cognito Pool ID로 — 현 라이브 값 `urn:amazon:cognito:sp:ap-northeast-2_wAdKW5HjX`. ⚠️ **console을 destroy→재apply하면 Pool ID가 새로 발급**되므로 그때마다 Entra 식별자 갱신 필요(SSO를 자주 켤 거면 console 레이어는 유지 권장).
5. **검증:** `https://cnapp-agentic.cloud` 접속 → 로그인 클릭 → Microsoft(Entra) 로그인 → 대시보드에 역할(viewer/approver) 표시.

> **코드 상태(2026-07-03):** `infra/console/domain-sso.tf`(신설)·`main.tf`(CloudFront 조건부 도메인)·`variables.tf`(`enable_custom_domain`·`domain_name`) 작성·validate 통과. **전부 `enable_custom_domain=false` 기본이라 현재 apply엔 무영향.** 남은 것 = DNS 전파 + 위 런북 + 프론트 OIDC 배선(앱 코드).
