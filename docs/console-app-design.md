...
| **Attack-path 그래프** | 노드(리소스)+엣지(관계) 시각화, **AWS/Azure 클라우드 경계를 시각적으로 구분(레인/박스)하고 경계를 넘는 크로스클라우드 엣지를 강조**, "공격자가 ~로 들어와 ~를 타고 Azure ~까지 탈취" 내러티브 텍스트 | UC3 |
...
- **크로스클라우드 엣지 강조:** 경계를 넘는 엣지(예: `member` 공개 S3(AWS) → Azure 공개 Blob)는 굵은/점선·강조색으로 그려 "여기서 클라우드를 넘는다"를 시각적으로 못 박음.
- **골든 시나리오 매핑:** target-app-design 3번의 1→2→3→4 경로가 AWS 레인(product→order→member)에서 시작해 마지막 엣지에서 Azure 레인(Blob)으로 건너가는 형태로 렌더 → 데모 하이라이트.
- 내러티브 텍스트도 "AWS에서 시작해 **Azure로 건너가** 민감데이터 전량 노출"처럼 경계 횡단을 명시.
...
## 12. infra/console 매핑 (Terraform 골격)
