...
| 변수 | `log_retention_days`·Bedrock 단가 4종·`bedrock_hourly_cost_alarm_usd`·`rds_connections_alarm_threshold`(`variables.tf`) ※ `grafana_alb_dns_name`·`grafana_alb_zone_id`는 external-dns 전환으로 **2026-07-09 제거됨** |
...
**만든 것:**
- **AWS Load Balancer Controller**(IRSA는 이 레이어 Terraform, 컨트롤러 본체는 `gitops/argocd/app-alb-controller.yaml`로 Helm 배포) — Karpenter 노드는 ASG가 아니라서 NodePort+수동 Target Group 등록이 안 통해서, 파드 IP를 직접 Target Group에 등록(IP 모드)하는 이 컨트롤러가 필요했다(현업 표준 패턴). **`vpcId`는 일부러 안 박음** — 컨트롤러가 노드 IMDS에서 자동 발견(shared 재apply로 VPC ID가 바뀌어도 하드코딩 stale 없음). replicaCount=1(프리티어 파드밀도 절약).
- **Grafana Ingress**(`gitops/monitoring/grafana-ingress.yaml`) — 이 Ingress 선언 하나로 컨트롤러가 전용 ALB를 자동 생성·관리(host-based 라우팅, `alb.ingress.kubernetes.io/target-type: ip`). **`certificate-arn`은 일부러 안 박음** — 컨트롤러가 host와 도메인이 일치하는 ISSUED ACM 인증서를 자동 발견(재apply로 ARN이 바뀌어도 stale 없음).
- **ACM 인증서**(이 레이어 Terraform, `aws_acm_certificate.grafana` + DNS 검증 레코드)만 TF 소유 — `grafana.cnapp-agentic.cloud` 서브도메인 인증서. 기존 `cnapp-agentic.cloud` 호스팅영역(CLI로 만든 영구 존)은 참조만 함.
- **Route53 A(ALIAS) 레코드는 external-dns가 자동 관리**(`gitops/argocd/app-external-dns.yaml`, IRSA는 이 레이어의 `aws_iam_role.external_dns`) — grafana Ingress를 감시해 ALB로 향하는 A 레코드를 실시간 생성·갱신. policy=upsert-only(삭제 안 함)+txt 레지스트리+domainFilter로 같은 존 다른 레코드는 안 건드림.

**✅ ALB DNS 하드코딩 문제 해소(2026-07-09).** 예전엔 `var.grafana_alb_dns_name`에 ALB DNS를 손으로 박아 Route53 A 레코드를 TF가 만들었는데, ALB 이름이 EKS/Ingress 재생성마다 랜덤(`k8s-monitori-grafana-...`)이라 매번 stale로 깨졌다. 이제 **external-dns가 그 값을 실시간으로 읽어 관리**하므로 수동 갱신이 필요 없다(그 변수도 제거함). 마찬가지로 vpcId·certificate-arn도 자동 발견으로 전환해 비결정값 하드코딩 3종을 전부 없앴다. ⚠️ 단, external-dns는 upsert-only라 monitoring destroy 후에도 grafana A·TXT 레코드는 존에 남는다(무해 — 다음 apply 때 TXT 소유자가 일치해 자동 재채택·갱신).
...
*(이 표의 두 항목은 완료돼 아래 "완료된 항목"으로 이동 — 현재 순수 미결/보류는 없음)*
