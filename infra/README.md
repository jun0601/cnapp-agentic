...
2. **apply 순서 = ① shared → ② karpenter(클러스터가 있어야 함) → ③ (target | backend | console — shared만 참조라 병렬 가능) → ④ monitoring(shared+backend+console 참조라 마지막) → 검증 → 역순 destroy(monitoring → console·backend·target → karpenter → shared).** ← **이 순서를 `infra/deploy.ps1`이 강제**(§3.5). **`monitoring`은 2026-07-22부터 'all'에 포함**(jh_lee가 전 terraform 소유) — destroy 시 `Clear-GrafanaIngress` 훅이 ALB 컨트롤러가 살아있을 때 Grafana Ingress를 먼저 지워 ALB 고아를 막는다(2026-07-08 사고 자동 방지). 상시 유지가 필요하면 `$ApplyOrder`에서 `'monitoring'`만 빼면 `all` destroy에서 제외된다.
...
./infra/deploy.ps1 -Action apply                     # 'all' = shared→karpenter→target→backend→console→monitoring (각 수동 승인)
./infra/deploy.ps1 -Action apply   -AutoApprove      # 승인 생략
./infra/deploy.ps1 -Action destroy -AutoApprove      # 역순: console→backend→target→karpenter→shared (monitoring 제외)
./infra/deploy.ps1 -Action apply   -Layer shared     # 단일 레이어만
./infra/deploy.ps1 -Action apply   -Layer monitoring # monitoring은 별도(진우 소유) — 'all'에 안 들어감
./infra/deploy.ps1 -Action apply   -Layer console -ExtraArgs '-var','enable_custom_domain=true'  # SSO 커스텀도메인
```

- **동작:** 각 레이어 폴더로 `Push-Location` 후 `terraform init -reconfigure`(+validate/plan/apply/destroy). `-chdir=<한글 절대경로>`를 인자로 넘기지 않는다 — PowerShell→exe 인자 인코딩에서 한글 경로가 깨지기 때문.
- **karpenter 헬스게이트(자동):** karpenter apply 직후 `kubectl rollout status deployment/karpenter`로 컨트롤러가 실제 Ready인지 게이트한다 — helm `wait=true`는 시작-즉시-panic형 CrashLoop을 통과시켜 **terraform green ≠ 컨트롤러 healthy**였던 실측(2026-07-03, 1.1.1×K8s1.34) 재발 방지. 실패 시 파드 상태·로그 20줄을 찍고 중단(kubectl 없으면 경고 후 스킵).
