...
2. **apply 순서 = ① shared → ② karpenter(클러스터가 있어야 함) → ③ (target | backend | console — shared만 참조라 병렬 가능) → 검증 → 역순 destroy(console·backend·target → karpenter → shared).** ← **이 순서를 `infra/deploy.ps1`이 강제**(§3.5). **⚠️ `monitoring`은 `deploy.ps1`의 'all' 스윕에서 제외**(진우/jw_kim 소유·별도 운영, 2026-07-07) — 개별 실행 `-Layer monitoring`. monitoring은 shared+backend+console 참조라 넣으면 apply 마지막/destroy 처음이 되지만, 소유 분리를 위해 'all'에서 뺐다(되돌리려면 `deploy.ps1`의 `$ApplyOrder`에 `'monitoring'` 한 줄 추가).
...
./infra/deploy.ps1 -Action apply                     # 'all' = shared→karpenter→target→backend→console (monitoring 제외·각 수동 승인)
