...
- **참조 방향:** 상위 레이어가 `terraform_remote_state`로 하위 레이어의 **출력(outputs)만 읽는다**(수정 불가) — 대부분 shared만, `monitoring`만 shared + backend·console도 참조. → shared가 반드시 **먼저**, monitoring이 **마지막** apply.
...
| `backend/` | **수집·정규화**(SQS·ingest/normalize Lambda) + **상관·오케스트레이터** Lambda(2-pass) + **조치** remediation SFn·감사 Object Lock 버킷 — *구 pipeline+engine 병합* | 무료급 + Bedrock 호출당 | ✅ validate (〃) |
| `console/` | S3+CloudFront(OAC)·Cognito(Entra SAML)·ALB(authenticate-cognito)→Lambda | ALB ~$0.03/h·CF/Lambda 무료급 | ✅ validate (〃) |
| `monitoring/` | Grafana IRSA·CloudWatch 대시보드 17위젯·CloudTrail→Logs 배관·Teams 알림(SNS·Lambda·알람6) | 무료급(대시보드·로그량) | ✅ validate (**shared+backend+console 후**) |
...
2. **apply 순서 = ① shared → ② (target | backend | console — shared만 참조라 병렬 가능) → ③ monitoring(셋 뒤, 마지막) → 검증 → 역순 destroy(monitoring → 셋 → shared).**
   ⚠️ **의존 레이어의 plan은 그 하위 레이어 apply 후에만 가능** — `terraform_remote_state`가 실제 S3 state를 읽기 때문(하위 state 없으면 outputs가 비어 참조 에러 = 코드 버그 아님·구조적 제약). 특히 monitoring plan은 backend·console state가 있어야 함.
