...
- ▶ 다음(객관 추천): ① **싼 CI**(GitHub Actions로 run_demo·run_e2e·validate 자동 실행 + Trivy/Checkov 게이트, $0·Shift-Left·회귀 방지) ② **실 스캐너 1회**(scan_securityhub/scan_prowler/trivy — 실계정, apply 게이트) ③ console-backend(TS Lambda). *CD/풀 인프라 상시 apply는 비용상 보류.*
