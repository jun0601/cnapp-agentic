...
- ▶ 다음(코드=무비용): (보너스) remediation Lambda 실코드·감사 S3 Object Lock. **그다음 = 실전환 apply**(준형이 apply 위임 — 단계별 apply→검증→destroy, EKS 첫 apply 직전 1회 확인). SSO 라이브는 도메인 결정 후(apply 때 재논의).
...
- ▶ 다음: 워크로드 스캐너 나머지(Inspector·kube-bench, infra apply 후) · App Registration(SSO)(infra/console 선행) · Defender(데모 때만)
