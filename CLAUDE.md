...
*진우*
- ✅ 한 것: TF state 버킷 · Cognito SSO 설계(authenticate-cognito·그룹 클레임) · 2-pass 트리거 이벤트 · Azure/Entra 테넌트 · **엔진 Hypothesis·Reasoning·Orchestrator** · **Azure App Registration 3종(과도권한 overpriv-app·order-sp·prowler-sp) + Prowler SP GitHub Federated Credential**(manual-infra §3.6) · **정규화부(`pipeline/normalize`) — ASFF·prowler·trivy→OCSF-lite(dedup·control 역인덱스, 목업 동작)**
- ▶ 다음: App Registration(SSO)+그룹(infra/console 선행) · 워크로드 스캐너 · 수집부(ingest, 진우 정규화부와 계약⑤로 이음) · Defender(데모 때만)
