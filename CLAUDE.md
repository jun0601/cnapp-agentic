...
- ▶ 다음(코드=무비용, 남은 것): ① **infra/console·pipeline·engine terraform**(apply 세션 대비 배포 코드) ② (보너스) remediation SFn. **그다음 = 실전환 apply**(준형이 apply 위임 — 단계별 apply→검증→destroy로 준형이 돌리고 로그·문서로 학습되게).
