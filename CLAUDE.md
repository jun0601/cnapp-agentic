...
- ✅ **Phase2 배선(2026-07-02) — end-to-end 러너 `run_e2e.py`**: 스캐너(Trivy)→정규화→상관(attack-path)→엔진→RAG를 한 줄로 관통. 스캐너발 CVE가 파이프라인 끝(엔진 confirmed·RAG 설명)까지 도달(exit 0). 정규화·상관 실 코드 관통 / 스캐너=실 trivy 출력 동일 구조 JSON(라이브 `trivy image`는 trivy 미설치라 대기) / 엔진·RAG=무비용 Mock(실 tool-use는 Phase1).
- ▶ 다음(객관 추천): ① **싼 CI**(GitHub Actions로 run_demo·run_e2e·validate 자동 실행, $0·Shift-Left) ② **실 trivy 스캔 1회**(trivy 설치 또는 CI에서) ③ console-backend(TS Lambda). *CD/풀 인프라 상시 apply는 비용상 보류(apply→destroy 사이클 유지).* 준형 희망(인프라/CI-CD)은 CI(싼 쪽)부터 = 정합.
