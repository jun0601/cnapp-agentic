...
**[레이아웃 스케치]** — 도식이 "겹겹의 층"을 보여주고, 카드가 각 층의 상세를 맡는 **1:1 대응 구조**.
```
┌────────────────────────┬──────────────────────────┐
│   심층 방어 도식          │ 카드1 Read-only First      │
│  ┌──────────────────┐  │ 카드2 HITL 승인 경로        │
│  │ ① CloudFront+WAF │  │ 카드3 불변 감사로그         │
│  │ ┌──────────────┐ │  │ 카드4 키리스 인증           │
│  │ │ ② SSO+JWT    │ │  │ 카드5 CloudFront WAF       │
│  │ │ ┌──────────┐ │ │  │ 카드6 SSO 페더레이션        │
│  │ │ │③ read-only│ │ │  │                          │
│  │ │ │ ┌──────┐  │ │ │  │  ← 6개를 2열×3행으로 깔면    │
│  │ │ │ │④HITL │  │ │ │  │     세로 여백이 덜 남는다     │
│  │ │ │ │ ▣중심 │  │ │ │  │                          │
│  └──┴─┴─┴──────┴──┴─┘  │                          │
│  하단 관통 밴드: 키리스 · 불변 감사로그                   │
└────────────────────────┴──────────────────────────┘
```
> 카드 순서를 **도식의 층 번호와 맞춘다**(①→카드1 …). 안 맞으면 두 요소가 따로 노는 그림이 된다. 중심 ▣ = 보호 대상(AWS 워크로드·Azure 신원). 하단 밴드는 **전 계층을 가로지르는** 것이라 층 안에 넣지 말고 도식 아래를 관통시킨다.

**▶ 캔바에 그대로 넣을 카피:**

[제목] 보안 거버넌스 & 플랫폼 자기방어

[중제목] 제품이 타깃에서 잡는 결함을 플랫폼 자신에게도 적용 — JWT 서명 미검증 권한상승 취약점을 스스로 발견·봉합
