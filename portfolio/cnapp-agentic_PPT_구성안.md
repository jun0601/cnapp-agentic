...
[도식 캡션 — Triage 비용 밸브 도식 위] Triage 게이트 — 비싼 단계(LLM) 앞에 싼 단계(규칙)를 세운다
  → **도식 왼쪽 끝에 맞춰 좌측정렬 · 작게 · 세미볼드.** 가운데정렬하면 제목처럼 보여 표 헤더와 위계가 헷갈린다.
  → 캔바 크롭이 draw.io 원본의 헤더 문장("비싼 단계(LLM) 앞에…")을 잘라내므로 **캡션이 그 역할을 대신해야 한다**. 이름(Triage 게이트) + 주장 조합이라 표 1행과 이름이 이어져 "저 행을 펼친 게 이거구나"가 자동으로 읽힌다.
  ※ 대안: `Triage 게이트 = 비용 밸브`(짧게) · `비싼 단계(LLM) 앞에 싼 단계(규칙)를 세운다`(주장만)
