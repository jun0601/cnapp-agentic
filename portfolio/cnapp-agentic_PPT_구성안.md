...
> ✍️ 톤 = **인사팀도 읽는 포폴**이라 날 코드 토큰(`?.`·`xmax=0` 등)은 평문으로 번역하되, 신뢰가는 기술 용어(Security Hub·Bedrock·Karpenter·심각도·destroy)는 남긴다.
...
· 증상: 보안 서비스(Security Hub)를 켜자 AI 조사 호출이 시간당 98 → 528건으로 5배 폭증
· 원인: 메시지를 모아 처리하도록 했지만 실제로는 1건씩 즉시 처리돼 엔진을 과다 기동 + 조사 대상 없는 배치까지 엔진을 깨움 + 이미 본 건이 다시 들어와도 걸러지지 않음
· 조치: 30초 단위 배치 처리 + 실제 조사 대상이 있을 때만 엔진 기동 + 이미 처리한 건은 재기동에서 제외(멱등 처리)
· 결과: 5분당 60 → 1회 (유입량은 그대로, 불필요한 기동만 제거)
· 핵심: 비용 통제의 축은 "무엇을 조사할지"만이 아니라 "얼마나 자주 실행할지"에도 있었다

[사례 2 · 보안 정합성 — 보안 등급이 실행 순서에 좌우]
· 증상: Critical로 설계한 항목이 다른 스캐너 결과와 합쳐지는 순간 High로 조용히 강등
· 원인: 여러 스캐너 결과를 병합할 때 심각도를 "나중에 들어온 값"으로 덮어써, 처리 순서가 최종 등급을 결정
· 조치: 병합 시 항상 더 높은 심각도를 유지하도록 규칙 수정 (모든 다중 소스 항목에 적용)
· 결과: 스캐너 처리 순서와 무관하게 최고 심각도 보장
· 핵심: 병합의 승자는 "마지막에 온 값"이 아니라 "더 위험한 값"이어야 한다

[사례 3 · 인프라 수명주기 — 고아 노드가 destroy를 막음]
· 증상: 인프라 삭제(destroy)가 보안그룹 의존성 오류로 반복 실패
· 원인: 오토스케일러(Karpenter)가 노드를 회수하기 전에 먼저 삭제돼, 남은 고아 노드의 네트워크 자원(ENI)이 보안그룹을 붙잡고 있었음
· 조치: 배포 스크립트에 "삭제 직후 고아 노드와 잔여 네트워크 자원을 자동 정리"하는 단계 추가
· 결과: 수동 복구 2회 → 완전 자동화, 재발 0
· 핵심: 분산 시스템의 자원 회수는 순서가 전부 — 재발 방지는 문서가 아니라 코드로

[사례 4 · 앱/데이터 정합 — 실데이터에서만 터진 크래시]
· 증상: 테스트(목업)에선 정상인데 실 DB 연동 모드에서만 상세 화면이 크래시
· 원인: 백엔드 쿼리가 화면에 필요한 일부 필드를 빠뜨려, 없는 값을 참조하다 화면 전체가 깨짐 (목업은 전 필드, 실 쿼리는 선택 필드라 차이가 숨어 있었음)
· 조치: 쿼리가 필요한 필드를 모두 조회하도록 수정 + 프론트 방어 코드 추가 + 전 페이지(9개) 렌더 검증
· 결과: 크래시 → 9개 페이지 전부 정상
· 핵심: "목업이 통과했다"가 "실데이터가 통과한다"는 아니다 — 두 경로를 필드 단위로 대조
