...
- **[카드 2] 🤖 AI 자율 조사·설명 (이 제품의 차별점)**
  - Evidence(능동조사) 탭: AI가 스스로 호출한 read-only API와 판정 근거를 타임라인으로 표시 → 10p(Bedrock 실증)
...
**▶ 캔바 카피 (v11) — [A] 카피만 복붙 / [B] 볼드·색 표 / [C] 제작 메모(슬라이드 밖)**

━━ [A] 슬라이드에 들어갈 카피 (이 블록만 복붙, 대괄호 라벨은 위치 안내라 빼고) ━━

[제목] 관제 콘솔 소개 — 전체 화면 + 기능별 상세 포인터
[중제목] 취약 app을 감시하는 관제 콘솔 — 스캔·정규화·AI 조사·설명, 프로젝트의 모든 기능이 하나로 모이는 화면
[구현 개요] React SPA(Vite+TS, S3+CloudFront 정적 호스팅) ↔ ALB → TypeScript Lambda → RDS pgvector(실 데이터) · Cognito SSO(Entra SAML 페더레이션, viewer/approver RBAC) · 커스텀 도메인 운영 · 9개 화면
[스크린샷 캡션] 실배포 화면 — 커스텀 도메인(cnapp-agentic.cloud) · Entra SAML SSO 로그인(APPROVER) · 실 RDS 데이터 기반

카드 1 — 탐지·현황
· 대시보드: Secure Score·심각도별 KPI·크로스클라우드 attack-path 배너를 한 화면에 요약 → 19p
· Findings 목록: 스캐너가 올린 finding을 필터·정렬하고 AI 조사 상태를 컬러로 표시 → 12p

카드 2 — AI 자율 조사·설명 (차별점)
· Evidence(능동조사) 탭: AI가 스스로 호출한 read-only API와 판정 근거를 타임라인으로 표시 → 10p
...
━━ [B] 볼드·색 (복붙 후 캔바에서 지정) — 색은 액센트 1개·빨강 1개만 ━━

| 위치 | 볼드 | 색 |
|---|---|---|
| 중제목 | 모든 기능이 하나로 모이는 화면 | — |
| 카드1 | Secure Score · attack-path 배너 | — |
| 카드2 | 스스로 호출한 read-only API(최우선) · pgvector 근거 검색 | (차별점) = 액센트 |
| 카드3 | 위험도순 · Bedrock 24h 사용량 | 빨강 글자 = 빨강 |
| 카드4 | approver 승인 → 실제 조치(HITL) · 불변 로그(S3 Object Lock) | — |
| 완성도 | 실 RDS 조회(목업 아님) | — |

최우선 3개(먼저 눈에 들어와야): ① 카드2 "스스로 호출한 read-only API" ② 카드4 "실제 조치(HITL)" ③ 중제목 "모든 기능이 하나로 모이는 화면"

━━ [C] 제작 메모 (슬라이드엔 안 들어감) ━━
- 레이아웃: 왼쪽 = 대시보드 스크린샷 1장(assets/shot-console-dashboard.png) + 그 아래 캡션 / 오른쪽 = 카드 4장 세로로 쌓기, 맨 아래 완성도 문구. 각 카드 = 별개 박스(카드끼리는 불릿 아님).
- 카드2 = 차별점이라 강조 최대 / 카드4 = 위계 한 톤 낮춤 / 컴플라이언스 = 단독 슬라이드 없어 페이지 참조 없음(의도).
...
> 🎨 디자인 = **4장 카드**, 각 카드 세로로 [증상 → 원인 → 조치 → 결과 → 교훈]. before→after 수치는 색 강조.

[제목] 트러블슈팅 — 내가 직접 부딪힌 문제 4건

[중제목] 비용·보안·인프라·데이터 — 갈래가 다른 4건을 직접 규명·해결

[사례 1 · 비용/아키텍처 — Bedrock 호출 5배 폭증, 엔진 재기동에 통제가 없었다]
· 증상: Security Hub를 켜자 Bedrock 호출이 시간당 98→528 (5×)
· 원인: SQS batch_size=10인데 배칭 window=0이라 메시지 1건마다 엔진 기동 + suppressed뿐인 배치도 발행 + 재유입 finding이 게이트를 재통과(멱등성 부재)
· 조치: 배칭 window 30s + actionable finding 있을 때만 발행 + upsert RETURNING(xmax=0)로 "처음 들어온 open"만 기동
· 결과: 5분당 60→1 (유입은 그대로, 엔진만 안 깨어남)
· 교훈: 트리아지 게이트는 "무엇을 조사할지"만 막는다 — "얼마나 자주 깨울지"는 별개 축이었다

[사례 2 · 보안 정합성 — 보안 등급이 스캐너 실행 순서에 좌우되던 버그]
· 증상: Critical로 설계한 control이 Macie High 라벨과 병합되는 순간 조용히 강등
· 원인: UPSERT가 sources는 누적하면서 severity_id는 EXCLUDED로 단순 덮어써서, 나중에 처리된 스캐너가 등급을 결정
· 조치: LEAST(기존, 신규)로 항상 더 심각한 쪽 유지 (모든 다중소스 control)
· 결과: 실행 순서와 무관하게 max severity 보장
· 교훈: 다중소스 병합은 "마지막이 이김"이 아니라 "더 심각한 게 이김"

[사례 3 · 인프라 수명주기 — 고아 노드가 destroy를 막던 문제]
· 증상: destroy가 노드 SG DependencyViolation으로 반복 실패
· 원인: 컨트롤러가 노드를 회수하기 전에 삭제돼 고아 스팟노드가 남고, 그 pod ENI(aws-K8S-*)가 GC되지 못하고 노드 SG를 붙잡음
· 조치: deploy.ps1 훅에 "고아 노드 종료 + available pod ENI 삭제(in-use는 불건드림)" 자동 스윕 추가
· 결과: destroy 수동복구 2회 → 자동화, 재발 0
· 교훈: teardown은 순서가 전부 — 재발 방지를 문서가 아니라 코드로

[사례 4 · 앱/데이터 정합 — 목업엔 없고 실데이터에서만 터진 크래시]
· 증상: 실 RDS 모드에서만 Finding 상세 크래시 + Evidence 탭 반쪽 (목업엔 정상)
· 원인: 실쿼리가 sources·case 필드를 빠뜨려 undefined.join() (mock=전 필드 vs real=선택 컬럼 2경로)
· 조치: 실쿼리 전 필드 SELECT + 프론트 방어(?.) + playwright 9/9 페이지 렌더 감사
· 결과: 크래시 → 9/9 정상 렌더
· 교훈: 목업 통과 ≠ 실데이터 통과 — 두 경로를 항상 필드 단위로 대조
