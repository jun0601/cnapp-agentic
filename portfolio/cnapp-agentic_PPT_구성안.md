...
│ 경량 대체        │ 관리형 편의보다 비용      │ NAT $32→$3(90%↓) │     '효과' 열 우측정렬
│ Mock-first      │ 인프라 없이 2인 병렬     │ 빌드기간 미가동    │
│ apply→destroy   │ 상시 가용성 포기        │ 6레이어 순서 강제  │
│ 모델 티어링·게이트 │ 승급된 것만 조사        │ 20건→12건·1회 100원│
│ ★비용 가시성     │ 쓰는 만큼 보여야 통제됨   │ 알림·알람·토큰 귀속 │  ← 이 행이 FinOps 차별점
└──────────────────────────────────────────────────────────┘
        ┌───────────────────────────────────┐
        │  NAT 월 ~$32 → ~$3  (약 90%↓)      │  ← 빅넘버 1개만
        └───────────────────────────────────┘
   [깊이 한 줄 — 크레딧이 실사용을 가리던 함정 발견·교정]
                  [핵심 원칙 — 하단 1줄]
```
> **빅넘버는 하나만.** 표에 이미 숫자가 4개 있어서 아래에 또 여러 개를 늘어놓으면 어디가 핵심인지 사라진다. "$0"·"무료"는 쓰지 않는다(구독형 보안 서비스를 데모에 켜면 실제로 과금되므로 정직하지 않다).
...
[중제목] 경량 대체 · LLM 호출 전 규칙 게이트 · apply→destroy 자동화로 비용을 아키텍처에 내장하고, 실사용·크레딧·순액 3축 관측으로 소비를 상시 추적
...
[깊이 한 줄 — 표 아래] 일일 비용 알림을 붙이고 보니 순액이 매일 $0으로 보이는 함정을 발견 — 프로모션 크레딧이 실사용을 100% 상쇄하고 있었다(실측: 순액 ≈ 0인데 실사용 +0.38 USD). 알림을 실사용·크레딧·순액 3줄로 교정.
