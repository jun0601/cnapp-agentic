...
> **"멋있게 들리지만 아무것도 안 알려주는 문장"을 쓰지 않는다.** 그 자리에 **동작·경로·수치**를 넣는다. 읽는 사람이 엔지니어라, 슬로건은 넘기고 구체어에서만 멈춘다.
>
> | ❌ 이렇게 쓰지 말 것 | ✅ 이렇게 | 왜 |
> |---|---|---|
> | Git이 원하는 상태의 **단일 진실** | self-heal이 kubectl로 직접 바꾼 값을 Git 상태로 되돌림 | GitOps 홍보 문구 → 실제 동작으로 |
> | 결과를 파이프라인에 **자동 합류** | S3 → ingest Lambda → findings | 모호한 동사 → 경로로 |
> | **이질 포맷** 통일 | ASFF·OCSF·prowler·trivy·kube-bench 6종 → OCSF-lite | 뭉뚱그림 → 실제 이름으로 |
> | **완성형** 관제 앱 | 전 화면이 목업 아닌 실 RDS 조회 | 자화자찬 → 검증 가능한 사실로 |
> | **사일로 없이** 서로 이해 | 상대 영역 코드를 리뷰·수정한 커밋이 양쪽에 남아 있음 | 컨설팅 용어 → 증거로 |
> | 자기 자신부터 **안전해야 하는 만큼** | JWT 서명 미검증 권한상승 취약점을 스스로 발견·봉합 | 훈화조 → 실제 사례로 |
...
[중제목] 9개 영역을 반씩 나누고, 컴포넌트 이음새를 JSON 계약으로 먼저 고정해 서로를 기다리지 않고 병렬 개발
...
[하단 문구] 나눈 뒤에도 상대 영역 코드를 서로 리뷰하고 직접 고쳤다 — 상대가 만든 상관 로직·정규화 파서의 버그를 찾아 수정한 커밋이 양쪽 모두에 남아 있다.
...
· 정규화 — ASFF·OCSF·prowler·trivy·kube-bench 6종 포맷 → OCSF-lite 한 스키마로
...
[완성도 문구] 총 9개 화면 — 위 3장 카드가 쇼케이스 화면 6개(뒤 슬라이드로 연결), 맨 아래 카드가 실무 화면 3개(컴플라이언스·조치·감사). 전 화면이 MSW 목업이 아니라 실 RDS를 조회하며, 커스텀 도메인에서 SSO 로그인 후 동작한다.
...
[중제목] 제품이 타깃에서 잡아내는 결함을 플랫폼 자신에게도 적용 — JWT 서명 미검증으로 인한 권한 상승 취약점을 스스로 발견해 봉합했다
