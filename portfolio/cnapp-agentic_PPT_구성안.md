...
- ✅ **결론(2026-07-23 렌더 확인) — 도식 1개로 확정, 대신 폭을 85~90%로 키운다.** 표가 이미 실측 숫자를 다 지고 있어서 아래는 **가장 센 주장 하나만 펼치면** 균형이 맞는다(2개를 넣으면 표가 밀린다). 다만 첫 렌더에서 도식이 가로 60%만 쓰고 좌우에 큰 여백이 남아 **"내용이 부족해 띄운 것"처럼 보였다** — 폭을 키우면 ⓐ 헐렁함이 사라지고 ⓑ 게이트 안 조건식(`severity_id ≤ 2 OR attack_path_id ≠ null`)이 비로소 읽힌다(**"규칙으로 거른다"의 유일한 증거라 반드시 읽혀야 함**). 2단 증거 레이아웃은 폐기.
- ⚠️ **엄밀히 말하면 이 도식은 '증거'가 아니라 '설명'이다**(직접 그린 그림이지 스크린샷이 아니므로). 표에 실측 숫자가 이미 있어서 설명 하나로 균형이 맞지만, **진짜 증거를 하나 더 넣는다면 정답은 콘솔 `/system`의 "Bedrock 사용량 24h" 패널 크롭** — 표에서 유일하게 증거가 없는 `비용 가시성 자동화` 행을 정확히 메우고, **우리 제품이 직접 비용을 관측하는 화면**이라 "제품에 내장했다"가 강하게 읽힌다(실값 예시 `호출 1,033회 · 입력 1,219,861 토큰 · 출력 219,479 토큰`). **재캡처가 필요하므로 지금은 넘어가고, 포폴 사이트용 `console-system.png`를 다시 찍을 때 같이 확보한다.**
...
└──────────────────────────────────────────────────────────┘
   💡[깊이 한 줄 — 크레딧이 실사용을 가리던 함정 발견·교정]
        ┌───────────────────────────────────────┐
        │   Triage 비용 밸브 도식 (가운데 정렬)      │  ← 빅넘버 박스 없음
        └───────────────────────────────────────┘
                  [핵심 원칙 — 하단 1줄]
