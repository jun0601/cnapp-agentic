...
- 📸 **시각자료 ② → 콘솔 `/system`(AI·시스템)의 "Bedrock 사용량 24h" 패널 크롭**: 표의 가장 약한 행(`비용 가시성 자동화`)을 실물로 증명. 실값 예시 `호출 1,033회 · 입력 1,219,861 토큰 · 출력 219,479 토큰`. **우리 제품이 직접 비용을 관측하는 화면**이라 외부 알림 스샷보다 "제품에 내장했다"가 강하게 읽히고, 표의 `케이스별 토큰 귀속(EMF)` 주장과 화면이 정확히 대응한다. (포폴 사이트용 `console-system.png` 재캡처와 **같은 캡처를 재사용**하면 됨)
  - ~~Teams `cnapp-cost` 일일 비용 알림 캡처~~ — 이전 프로젝트에서 이미 쓴 소재라 재탕 회피(2026-07-23 결정). 크레딧 상쇄 함정 스토리는 **텍스트 한 줄**로만 유지한다.
  - **넣지 않아도 무방** — Triage 비용 밸브 도식 하나만으로도 "주장→증거" 전환은 성립한다. 이 경우 표 아래를 1단(도식 전폭)으로 쓰면 더 깔끔하다.
```
┌──────────────────────────────────────────────────────────┐
