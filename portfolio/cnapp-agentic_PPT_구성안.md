...
- 📸 **시각자료 ② → Teams `cnapp-cost` 채널 일일 비용 알림 캡처**: 표의 가장 약한 행(`비용 가시성 자동화`)을 실물로 증명. **실사용·크레딧·순액 3줄**이 보이게 캡처 — 그 3줄 포맷 자체가 "크레딧이 실사용을 가리던 함정"을 발견·교정한 결과물이라 캡션 한 줄로 스토리가 닫힌다.
```
┌──────────────────────────────────────────────────────────┐
