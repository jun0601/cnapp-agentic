...
| 전략 | 왜 골랐나(트레이드오프) | 효과 |
|---|---|---|
| 모델 티어링 + Triage 게이트 | 저가 모델 + 승급된 것만 조사(비용 통제를 제품에 내장) | 실측 **20건 → 12건만 승급** · 조사 1회 **~100원** |
| 비용 가시성 자동화 | 줄이는 것만으론 부족 — **쓰는 만큼 보여야 통제**된다 | 일일 비용 알림 · Bedrock 시간당 임계 알람 · **케이스별 토큰 귀속(EMF)** |
...
| Mock-first 개발 | 인프라 없이 2인 병렬 개발 — 계약 스키마를 먼저 고정하고 이후 실 연동으로 전환 | 빌드 기간 클라우드 인프라 미가동 |
...
[표 — 전략 / 왜 골랐나(트레이드오프) / 효과]  ※ 이 순서 그대로(AI 비용 통제 2행이 위)

| 전략 | 왜 골랐나(트레이드오프) | 효과 |
|---|---|---|
| 모델 티어링 + Triage 게이트 | 저가 모델 + 승급된 것만 조사(비용 통제를 제품에 내장) | 실측 **20건 → 12건만 승급** · 조사 1회 **~100원** |
| 비용 가시성 자동화 | 줄이는 것만으론 부족 — 쓰는 만큼 보여야 통제된다 | 일일 비용 알림 · Bedrock 시간당 임계 알람 · **케이스별 토큰 귀속(EMF)** |
| 경량 대체(NAT Instance·RDS t3.micro+pgvector) | 관리형 편의보다 비용 — 데모 규모엔 경량이 합리적 | NAT 월 ~$32 → ~$3 (**약 90%↓** ← 초록) |
| Apply → Test → Destroy | 상시 가용성 포기, 월 과금 → 시간 과금 | 배포 스크립트(deploy.ps1)가 6레이어 순서까지 강제 — 정리 누락 방지 |
| Mock-first 개발 | 인프라 없이 2인 병렬 개발 — 계약 스키마를 먼저 고정하고 이후 실 연동으로 전환 | 빌드 기간 클라우드 인프라 미가동 |
