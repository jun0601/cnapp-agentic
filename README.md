...
| 🔴 **실 findings 라이브 관통 (2026-07-06)** | ✅ **워크로드 스캐너 2종 실제 라이브 실행** — **kube-bench**(실 3노드 Job → CIS 4.1.1 cluster-admin 실 탐지)·**Trivy**(인클러스터 ECR 스캔 → 실 CVE 205개) → **실 `normalize` Lambda → RDS → 라이브 콘솔 23건**(골든 20 + 실 스캔 3). `INTERNAL-KSPM-CLUSTERADMIN-001` 신설·normalizer 노이즈 규칙 개선. ⬜ **Prowler(CSPM)는 opt-in 보류**(Python 3.8 CLI 불가 + 광역 read IRSA 필요 + 비큐레이티드 노이즈). Security Hub·Macie는 계정 구독제약이라 데모 직전 활성 |
