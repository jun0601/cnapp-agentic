...
| 🖥️ **관제 콘솔 (`apps/console`)** | ✅ **라이브 완성 + 비주얼 고도화(2026-07-06)** — 실데이터 렌더 + **SSO 실 로그인**(jh_lee@ APPROVER) + **AI 어시스턴트 챗**(`/chat`, RAG 실 답변·pgvector 근거) + **전 페이지 프리미엄 SaaS 톤 고도화**(디자인시스템·그라데이션 shell·Evidence 능동조사 타임라인 stepper 등 9화면). Vite+React+TS+Tailwind+TanStack+React Flow. `tsc`·`vite build`·playwright 무회귀 |
...
| ✅ **라이브 실전환 완료 (2026-07-06)** | 전 레이어 apply · **커스텀 도메인(cnapp-agentic.cloud)+ACM+SSO 실 로그인** · 데이터평면 관통(실 Bedrock tool-use) · RAG /chat · remediation · gitops(member/product/order Running) · **보안 하드닝 7종(#1~#7)** · **관제앱 비주얼 고도화+AI 챗** 다 라이브 검증. **유일 갭 = 실 findings** — Security Hub·Macie·Inspector가 **계정 구독제약**(`SubscriptionRequired`, free tier 아님·AWS Support 필요)으로 막혀 **Prowler(구독 불필요)로 실 CSPM 스캔**만 남음(현재 20건은 골든 시드) |
